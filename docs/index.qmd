...
A beta-binomial GLMM with experimental unit as a random intercept was the best supported model (lowest AIC). Full model coefficients are reported in @tbl-s2-fixed-effects. Refuge use was most affected by period with more kōura using the refuge as the experiment progressed. Refuge use was significantly more in the during period than in the before period (*p* = `r p_before_during`, @fig-emmeans-plot, @tbl-s3-emmeans). This was also the case between before and after period (*p* = `r p_before_after`), but not between during and after (*p* = `r p_during_after`). Neither treatment nor light condition significantly affected refuge use. No significant differences were found between catfish and eel (*p* = `r p_catfish_eel`), catfish and control (*p* = `r p_catfish_control`), or control and eel (*p* = `r p_control_eel`). Light condition was also not significant (*p* = `r p_dark_light`).

## Feeding experiment
The feeding experiment showed the highest mean (±SE) feeding rate was after 3 days of starvation in the catfish treatment (`r get_emm("catfish", "3")` ± `r get_se("catfish", "3")` pellets g⁻¹). The linear model showed starvation period had significant effect on feeding rate (*p* = `r p_day_starve`, @tbl-s4-feeding-anova). Treatment had no significant effect on feeding rate (*p* = `r p_treatment`), and the interaction between the treatment and starvation period was not significant (*p* = `r p_interaction`), although a marginal trend was observed (@fig-feeding-plot). No significant difference was found in feeding rate between catfish, eel, and control treatments within each of the starvation periods, this suggests that predator kairomones did not affect feeding behaviour in kōura within this experiment.
